...
const updateProfileSchema = Joi.object({
  name: Joi.string().min(3).allow("").optional().messages({
    "string.min": "Tên phải có ít nhất 3 ký tự",
  }),
...
const registerUser = async (req, res) => {
  try {
    ...
    const user = await patientModel.create({ email, password: hashedPassword, name });
    ...
    return res.status(200).json({
      ...
      data: {
        ...
        user: {
          ...
          name: user.name,
          email: user.email,
          image: user.image ? `${req.protocol}://${req.get("host")}/images${user.image}?t=${Date.now()}` : null,
...
const updateProfile = async (req, res) => {
  try {
    ...
    if (name !== undefined) {
      if (name === "") {
        console.warn("[updateProfile] Name cannot be empty for user:", userId);
    ...
    if (email !== undefined) {
      ...
      user.email = email || user.email; // Allow empty email to clear it if allowed by schema
    }

    if (phoneNumber !== undefined) {
      if (phoneNumber && !/^\+?\d{10,15}$/.test(phoneNumber)) {
        console.warn("[updateProfile] Invalid phone number format:", phoneNumber);
        return res.status(400).json({ success: false, message: "Định dạng số điện thoại không hợp lệ" });
      }
      user.phoneNumber = phoneNumber; // Allow empty string to clear the field
    }

    if (address !== undefined) {
      user.address = address; // Allow empty string to clear the field
    }
...
const updateProfileImage = async (req, res) => {
  try {
    ...
    if (!validTypes.includes(image.mimetype)) {
      console.warn("[updateProfileImage] Invalid file type:", image.mimetype);
      return res.status(400).json({ success: false, message: "Vui lòng chọn file ảnh (JPEG, PNG, hoặc WebP)" });
    }
    if (image.size > 5 * 1024 * 1024) {
      console.warn("[updateProfileImage] File too large:", image.size);
    ...
    if (user.image) {
      ...
      try {
        ...
        console.warn("[updateProfileImage] Error deleting old image:", err.message);
      }
    }

    user.image = `/uploads/misc/${image.filename}`;
    await user.save();
...
const getPublicDoctorData = async (req, res) => {
  try {
    ...
    const transformedDoctor = {
      ...
      image: doctor.image ? `${req.protocol}://${req.get("host")}/images${doctor.image}?t=${Date.now()}` : null,
    };
